...
const getAi = (): GoogleGenAI => {
    if (!aiInstance) {
        ...
        if (!apiKey) {
             // In browser environments with the polyfill, this might be empty string.
             // We throw here to be caught by the calling function, preventing app crash at startup.
...
export const generateVideo = async (prompt: string, image: { base64: string, mimeType: string } | null, aspectRatio: '16:9' | '9:16'): Promise<string> => {
    // We reuse getAi() here. Even though generateVideos is an operation, the client logic handles it.
    // Note: If you need a fresh client for race conditions, you can call getAi() again, 
    // but typically one instance is fine unless the key changes dynamically.
    const videoAi = getAi(); 
